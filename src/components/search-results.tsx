						...
						<div className="flex-1 min-w-0">
							<Link href={`/resource/${result.pinyin}`} className="block">
								<div className="flex items-center gap-2 flex-nowrap overflow-hidden">
									<Badge variant="outline" className="text-blue-600 bg-blue-50 shrink-0">
										{categoryMap.get(result.categoryKey)}
									</Badge>
									<h2 className="hover:underline truncate min-w-0 flex-1">{result.title}</h2>
								</div>
								<p className="mt-2 text-gray-600 text-sm break-words line-clamp-7 sm:line-clamp-none">{result.desc}</p>
